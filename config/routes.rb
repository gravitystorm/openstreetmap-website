OpenStreetMap::Application.routes.draw do
  ...
  # It's unclear what api_version should be in the base element for this call when
  # no version is in the url.
  namespace :api, :api_version => "0.6" do
    get "capabilities" => "capabilities#show"
  end

  scope "api" do
    # Methods unchanged between versions
    (API_VERSIONS & ["0.6", "0.7"]).each do |version|
      namespace v_string(version), :path => version, :module => :api, :api_version => version do
        ...
        # Map notes API
        resources :notes, :except => [:new, :edit, :update], :constraints => { :id => /\d+/ }, :defaults => { :format => "xml" }, :controller => "notes" do
          collection do
            get "search"
    ...
    # methods only available in 0.6
    (API_VERSIONS & ["0.6"]).each do |version|
      namespace v_string(version), :path => version, :module => :api, :api_version => version do
        # renamed in 0.7
    ...
    # methods only available in 0.7
    (API_VERSIONS & ["0.7"]).each do |version|
      namespace v_string(version), :path => version, :module => :api, :api_version => version do
        # renamed from 0.6
        get "tracepoints" => "tracepoints#index"

        post "trace/create" => "traces#api_create"
        get "trace/:id" => "traces#api_read", :id => /\d+/
        put "trace/:id" => "traces#api_update", :id => /\d+/
        delete "trace/:id" => "traces#api_delete", :id => /\d+/
        get "trace/:id/details" => "traces#api_read", :id => /\d+/
        get "trace/:id/data" => "traces#api_data"
      end
    end
  end
