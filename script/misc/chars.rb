...
# https://github.com/flopp/invisible-characters/blob/main/characters.json
...
chars.each do |char|
  c = char[:code].hex.chr(Encoding::UTF_8)
  width = Unicode::DisplayWidth.of(c)
  space_match = /\s/.match?(c)
  invalid_match = INVALID_CHARS.match?(c)
  if width != 0 && space_match == false && invalid_match == false
    puts "#{char[:code]} #{width} #{space_match} #{char[:name]}"
  end
end
